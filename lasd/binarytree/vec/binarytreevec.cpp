...
// Constructor
  ...
  BinaryTreeVec<Data>::NodeVec::NodeVec(Data&& elem, unsigned long i, unsigned long h, unsigned long l, unsigned long r, BinaryTreeVec<Data>* ref){
  ...
  template <typename Data>
  BinaryTreeVec<Data>::NodeVec::NodeVec(const Data& elem, unsigned long i, unsigned long h, unsigned long l, unsigned long r, BinaryTreeVec<Data>* ref){
    element = elem;
...
// Getters
  template <typename Data>
  const unsigned long BinaryTreeVec<Data>::NodeVec::getIndex() const{
    return index;
  }

  template <typename Data>
  const unsigned long BinaryTreeVec<Data>::NodeVec::getHeight() const{
    return height;
  }

  template <typename Data>
  const unsigned long BinaryTreeVec<Data>::NodeVec::getLeft() const{
    return left;
  }

  template <typename Data>
  const unsigned long BinaryTreeVec<Data>::NodeVec::getRight() const{
    return right;
  }
...
// Specific constructors
  template <typename Data>
  BinaryTreeVec<Data>::BinaryTreeVec(const Data& newelement){
    for(int i = 0; i < 100; i++)
      heightVector[i] = 0;
    ...
    treeVec[0] = new struct NodeVec(newelement, 0, 0, 1, 2, this);
  ...
  template <typename Data>
  BinaryTreeVec<Data>::BinaryTreeVec(Data&& newelement) noexcept{
    for(int i = 0; i < 100; i++)
      heightVector[i] = 0;
    ...
    treeVec[0] = new struct NodeVec(std::move(newelement), 0, 0, 1, 2, this);
  ...
  BinaryTreeVec<Data>::BinaryTreeVec(const BinaryTreeVec<Data>& copyTree){
    ...
    for(unsigned long i = 0; i < copyTree.treeVec.Size(); i++)
      if(copyTree.treeVec[i] != nullptr){
        treeVec[i] = new struct NodeVec(copyTree.treeVec[i]->element, copyTree.treeVec[i]->index,
...
// Specific member functions (inherited from BinaryTree)
  ...
  template <typename Data>
  void BinaryTreeVec<Data>::AddLeftChild(struct NodeVec& node, const Data& element){
    ...
    treeVec[node.left] = new struct NodeVec(element, node.left, node.height+1, 2 * node.left + 1,
        2 * node.left + 2, this);
  ...
  template <typename Data>
  void BinaryTreeVec<Data>::AddLeftChild(struct NodeVec& node, Data&& element){
    if(node.HasLeftChild())
      RemoveLeftChild(node);
    ...
    treeVec[node.left] = new struct NodeVec(std::move(element), node.left, node.height+1,
  ...
  template <typename Data>
  void BinaryTreeVec<Data>::AddRightChild(struct NodeVec& node, const Data& element){
    ...
    treeVec[node.right] = new struct NodeVec(element, node.right, node.height+1, node.right * 2 + 1,
        node.right * 2 + 2, this);
  ...
  template <typename Data>
  void BinaryTreeVec<Data>::AddRightChild(struct NodeVec& node, Data&& element){
    if(node.HasRightChild())
      RemoveRightChild(node);
    ...
    treeVec[node.right] = new struct NodeVec(std::move(element), node.right, node.height+1, node.right * 2 + 1,
        node.right * 2 + 2, this);
  ...
  void BinaryTreeVec<Data>::removeSubtree(struct NodeVec& node){
    unsigned long nodeIndex = node.index;

    unsigned long LIndex = node.left;
    unsigned long RIndex = node.right;
...
// Accessory functions
  ...
  void BinaryTreeVec<Data>::Expand(){
    unsigned long oldSize = treeVec.Size();
    treeVec.Resize(oldSize*2+1);

    for(unsigned long i = oldSize; i < treeVec.Size(); i++)
      treeVec[i] = nullptr;
  }
